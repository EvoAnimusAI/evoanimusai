...
sys.stdout = tee.Tee('logs/symbolic_log.txt', 'a', sys.stdout)
...
# === 🔁 Handler especializado para duplicar eventos simbólicos ===
class SymbolicLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if any(tag in msg for tag in ["[🧠", "[SYM]", "[⚖️", "[DECISION]", "[RULE]", "[EVAL]", "Evento simbólico", "Expresión simbólica"]):
            try:
                with open(self.filename, "a", encoding="utf-8") as f:
                    f.write(msg + "\n")
            except Exception as e:
                print(f"[❌ SYMBOLIC LOG ERROR] {e}")

symbolic_handler = SymbolicLogHandler("logs/symbolic_log.txt")
symbolic_handler.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] :: %(message)s"))
logger.addHandler(symbolic_handler)
# =================================================================
...
def main(daemon_key: str, test_mode: bool = False, resume: bool = False):
    ...
    contexto_critico = context_obj.state if context_obj and hasattr(context_obj, "state") else {}
